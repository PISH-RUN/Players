...
export const LoginAfter = ({phone}: LoginAfterProps) => {
    ...
    return (
      <Form form={form} onFinish={onFinish}>
          ...
          <Title type={Type.THIN} style={{fontSize: 16, margin: '16px auto'}}>لطفا کد ورود ارسالی را وارد نمایید</Title>
          <Row gutter={[10, 10]}>
              <Col xs={24}><PasswordInput placeholder="کد ورودیه" required/></Col>
              <Col sm={24} xs={24}><Button size="large" htmlType="submit" style={{width: "100%"}} loading={isLoading} type="primary">ورود به
                  سایت</Button></Col>
              {/*<Col sm={12} xs={24}><Button onClick={openScanner} size="large" type="primary">اسکن بارکد</Button></Col>*/}
          </Row>
          <Modal visible={scannerVisible} onCancel={() => setScannerVisible(false)} maskClosable footer={null}>
          </Modal>
      </Form>
    )
}
