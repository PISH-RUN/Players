...
const Users = (): JSX.Element => {
  ...
  const options = teams?.length > 0 ? [{ label: 'همه', value: 'all' }, ...teams.map((p: any) => ({
  ...
  useEffect(() => {
    ...
    const userPins = pins.filter((user) => present === 'all' || (present === 'present' ? !!user.attributes.enteredAt : !user.attributes.enteredAt)).map((user, index) => {
      return <AvatarPin key={index} userID={user.id} name={user.name} avatar={AvatarIcon} disabled={!user.attributes.enteredAt} />;
    });
    setPins(userPins);
    return () => setPins([]);
      ...
      <DashboardWrapper style={{ width: '30%', right: 0 }}>
        <Row style={{ height: '100%' }}>
          <Col md={24} style={{ paddingTop: 110, paddingRight: 75 }}>
            <MultiTitle title='همراهان پیش‌ران' subTitle='کیدنید' description={`${allUsers.filter(u => !u.attributes?.enteredAt).length} حاضر از ${allUsers?.length}`}
      ...
      <div className='custom-radio'>
        ...
        <Radio.Group options={options2} onChange={(e) => {
          ...
          setData(usersData[e.target.value]);
                     ...
                     value={present} className='radio-btn' />
      </div>
    </>
  );
};
