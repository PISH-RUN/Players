...
export const TaskStopWatch = (props: TaskStopWatchProps): JSX.Element => {
  ...
  const menu = (
    <Menu>
      <Menu.Item onClick={() => failTask(props.id)} key='1'>پایان ناموفق</Menu.Item>
    </Menu>
  );
            ...
            <Row justify='center' style={{ gap: 10 }} gutter={[10, 0]}>
              {
                started ?
                  ...
                  :
                  <Button style={{ width: '180px' }} type='primary' loading={isLoadingStartTask}
                          onClick={handleStartTask}>
                    آغاز وظیفه
                  </Button>
              }
              <Button style={{ width: '180px', marginLeft: 0 }} type={props.suspended ? 'primary' : 'default'} danger
                      loading={isSuspendLoading} onClick={handleSuspend}>مشکل
                خوردم</Button>
            </Row>
          </Col>
          : <Col xs={24}>{props.taskStatus === 'done' ?
            <Alert type="success" showIcon message='تسک با موفقیت انجام شده' />
            :<Alert type="error" showIcon message='متاسفانه انجام این تسک ناموفق بود' /> }</Col>}
      </Row>
    </div>
  );
};
